...
describe('전처리 판은 붙일 때도 뗄 때도 묻는다', () => {
   ...
   *
   * **`data/FullDataDialog.vue`는 `CLOSERS`에 안 들어간다** (2026-09-10). 위 목록이
   * 지키는 것은 *"묻고 나서 실패했을 때 창이 닫히는가"*인데, 이 창은 **묻지 않고
   * 실패하지도 않는다** — 이미 확정된 표를 쪽으로 나눠 보여주기만 하므로 프로젝트를
   * 건드리는 경로도, 잡을 예외도, 사라질 실험도 없다.
   */
